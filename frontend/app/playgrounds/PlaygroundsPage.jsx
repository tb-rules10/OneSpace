  ...
  useEffect(() => {
    ...
    // await new Promise(resolve => setTimeout(resolve, 3000));  
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pg?userId=${userId}`);
        if (!response.ok) {
    ...
    <>
      <div className="pt-32 pb-24 max-w-6xl mx-auto">
        ...
        <div className="mt-16">
          ...
          >
            {projects.length === 0 ? (
              <p className="text-zinc-400">No playgrounds found.</p>
            ) : (
              projects.map((project) => (
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between p-4 duration-150 border-t bg-zinc-900/40 border-zinc-500/30 group-hover:border-zinc-300/30">
                    <span className="text-sm text-zinc-500">{project.env}</span>
